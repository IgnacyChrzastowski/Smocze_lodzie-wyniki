                            ...
                            <form method="post">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        <input type="text" name="nazwa_wyscigu" id="addWyscigNazwa" class="form-control" required>
                                        <button type="button" class="btn btn-outline-secondary" id="btnAutoNazwa" title="Auto-numer wyścigu">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16"><path d="M8 3a5 5 0 1 0 4.546 2.914.5.5 0 0 1 .908-.417A6 6 0 1 1 8 2v1z"/><path d="M8 4.466V.534a.25.25 0 0 1 .41-.192l2.36 1.966c.12.1.12.284 0 .384L8.41 4.658A.25.25 0 0 1 8 4.466z"/></svg>
                                            Auto
                                        </button>
                                    </div>
                                    <div class="form-text">Kliknij „Auto", aby wstawić kolejny numer wyścigu.</div>
                                </div>
                                <div class="mb-3">
...
<script>
    ...
    (function () {
        ...
        // --- Auto-inkrement nazwy wyścigu ---
        (function () {
            var btn = document.getElementById('btnAutoNazwa');
            var inp = document.getElementById('addWyscigNazwa');
            if (!btn || !inp) return;
            btn.addEventListener('click', function () {
                // Zbierz wszystkie istniejące nazwy wyścigów z listy (data w DOM)
                var nums = [];
                document.querySelectorAll('.wyscig-row td:first-child').forEach(function (td) {
                    // numery id nie mają sensu — szukamy w 2. kolumnie nazw które są liczbami lub zaczynają się od liczby
                });
                // Lepsza metoda: pobierz nazwy z 2. kolumny tabeli wyścigów
                document.querySelectorAll('.wyscig-row').forEach(function (row) {
                    var cells = row.querySelectorAll('td');
                    if (cells.length >= 2) {
                        var txt = cells[1].textContent.trim();
